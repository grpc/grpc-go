...
// getDiscoveryMechanismName returns the name of the discovery mechanism.
//
// For EDS, it's the EDSServiceName (or ClusterName if empty).
// For DNS, it's the DNSHostName.
func getDiscoveryMechanismName(clusterName string, update xdsresource.ClusterUpdate) string {
...
//
// Each priorityConfig corresponds to one leaf cluster of the top-level cluster
// from the XDSConfig. If the top-level cluster is the leaf itself, then the
// priorityConfig corresponds to itself. If the cluster is of type EDS, edsResp
// will be set. If the cluster is of type DNS, endpoints will be set.
...
func buildPriorityConfig(priorities []priorityConfig, xdsLBPolicy *internalserviceconfig.BalancerConfig) (*priority.LBConfig, []resolver.Endpoint, error) {
	...
	for _, p := range priorities {
		...
		case xdsresource.ClusterTypeEDS:
			names, configs, endpoints, err := buildClusterImplConfigForEDS(p.childNameGen, p.edsResp, p.clusterName, p.clusterUpdate, xdsLBPolicy)
			if err != nil {
				return nil, nil, err
			}
			...
			continue
		case xdsresource.ClusterTypeLogicalDNS:
			name, config, endpoints := buildClusterImplConfigForDNS(p.childNameGen, p.endpoints, p.clusterName, p.clusterUpdate)
...
func buildClusterImplConfigForDNS(g *nameGenerator, endpoints []resolver.Endpoint, clusterName string, update xdsresource.ClusterUpdate) (string, *clusterimpl.LBConfig, []resolver.Endpoint) {
