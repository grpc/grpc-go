...
func Test(t *testing.T) {
	xdsdepmgr.EnableCDSEDSlooksups = true
...
func verifyXDSConfig(ctx context.Context, xdsCh chan *xdsresource.XDSConfig, errCh chan error, want *xdsresource.XDSConfig) error {
	...
	case update := <-xdsCh:
		// fmt.Printf("Eshita Got the update : %v", update)
		cmpOpts := []cmp.Option{
			cmpopts.EquateEmpty(),
...
// the XDSConfig.
// func TestAggregateClusterChildError(t *testing.T) {
// 	xdsdepmgr.EnableCDSEDSlooksups = true
// 	nodeID, mgmtServer, xdsClient := setup(t, true)
...
func (s) TestEndpointAmbientError(t *testing.T) {
	// xdsdepmgr.EnableCDSEDSlooksups = true
	// Expect a warning log for the ambient error.
	grpctest.ExpectWarning("EDS discovery mechanism for resource")
