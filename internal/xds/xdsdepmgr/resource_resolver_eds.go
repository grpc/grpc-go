...
func (er *edsDiscoveryMechanism) lastUpdate() (any, error, bool) {
	er.mu.Lock()
	defer er.mu.Unlock()

	if er.update == nil && er.err == nil {
		return nil, nil, false
	}
	if er.update != nil {
		return *er.update, nil, true
	}
	return nil, er.err, true
}
...
func (er *edsDiscoveryMechanism) AmbientError(err error, onDone func()) {
	...
	er.logger.Warningf("EDS discovery mechanism for resource %q reported ambient error: %v", er.nameToWatch, err)
	onDone()
}
