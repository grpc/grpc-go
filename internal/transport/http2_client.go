...
func newHTTP2Client(connectCtx, ctx context.Context, addr resolver.Address, opts ConnectOptions, onPrefaceReceipt func(), onGoAway func(GoAwayReason), onClose func()) (_ *http2Client, err error) {
	...
	if transportCreds != nil {
		...
		// Can ignore the boolean as the deadline will return the zero value,
		// which will make the conn not timeout on I/O operations.
		deadline, _ := connectCtx.Deadline()
		// Pull the deadline from the connectCtx, which will be used for
		// timeouts in the authentication protocol handshake.
		conn.SetDeadline(deadline)
		newConn, authInfo, err := transportCreds.ClientHandshake(connectCtx, addr.ServerName, conn)
		// After authentication handshake, take away the deadline set on the
		// connection.
