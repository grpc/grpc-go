...
func (p *testingPicker) Pick(info PickInfo) (PickResult, error) {
	if atomic.AddInt64(&p.maxCalled, -1) < 0 {
		return PickResult{}, fmt.Errorf("pick called to many times (> goroutineCount)")
	}
	if p.err != nil {
...
func (s) TestBlockingPickTimeout(t *testing.T) {
	...
	defer cancel()
	if _, _, err := bp.pick(ctx, true, PickInfo{}); status.Code(err) != codes.DeadlineExceeded {
...
func (s) TestBlockingPick(t *testing.T) {
	...
	for i := goroutineCount; i > 0; i-- {
		go func() {
			if tr, _, err := bp.pick(context.Background(), true, PickInfo{}); err != nil || tr != testT {
...
func (s) TestBlockingPickNoSubAvailable(t *testing.T) {
	bp := newPickerWrapper()
	var finishedCount uint64
	bp.updatePicker(&testingPicker{err: ErrNoSubConnAvailable, maxCalled: goroutineCount})
...
func (s) TestBlockingPickTransientWaitforready(t *testing.T) {
	bp := newPickerWrapper()
	bp.updatePicker(&testingPicker{err: ErrTransientFailure, maxCalled: goroutineCount})
	var finishedCount uint64
	// All goroutines should block because picker returns transientFailure and
	// picks are not failfast.
	for i := goroutineCount; i > 0; i-- {
		go func() {
			if tr, _, err := bp.pick(context.Background(), false, PickInfo{}); err != nil || tr != testT {
				t.Errorf("bp.pick returned non-nil error: %v", err)
			}
			atomic.AddUint64(&finishedCount, 1)
		}()
