...
func TestValidateChildPolicyConfig(t *testing.T) {
	...
	gotChildConfig, err := validateChildPolicyConfig(input)
	if err != nil || !cmp.Equal(gotChildConfig, wantChildConfig) {
		t.Errorf("validateChildPolicyConfig(%v) = {%v, %v}, want {%v, nil}", input, gotChildConfig, err, wantChildConfig)
	}
}
...
func TestValidateChildPolicyConfigErrors(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			input: &lbParsedConfig{
				childPolicy: &loadBalancingConfig{
					Config: []byte(`bad bad json`),
				},
			},
			wantErrPrefix: "rls: json unmarshal failed for child policy config",
		},
		{
	...
	for _, test := range tests {
		t.Run(test.desc, func(t *testing.T) {
			gotChildConfig, gotErr := validateChildPolicyConfig(test.input)
			if gotChildConfig != nil || !strings.HasPrefix(fmt.Sprint(gotErr), test.wantErrPrefix) {
				t.Errorf("validateChildPolicyConfig(%v) = {%v, %v}, want {nil, %v}", test.input, gotChildConfig, gotErr, test.wantErrPrefix)
			}
		})
	}
}
