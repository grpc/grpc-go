...
func Get(name string) Builder {
	if !envconfig.CaseSensitiveBalancerRegistries {
		...
		if lowerName != name {
			logger.Warningf("Balancer retrieved for name %q. grpc-go will be switching to case sensitive balancer registries. After a release, env variable will be removed", name)
		}
		name = lowerName
	}
	if b, ok := m[name]; ok {
