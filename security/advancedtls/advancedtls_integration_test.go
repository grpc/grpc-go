...
func (s) TestEnd2End(t *testing.T) {
	...
	for _, test := range []struct {
		...
		// by ServerTrust2.
		{
			...
			clientVerifyFunc: func(_ *HandshakeVerificationInfo) (*PostHandshakeVerificationResults, error) {
				return &PostHandshakeVerificationResults{}, nil
			},
			clientVerificationType: CertVerification,
			serverCert:             []tls.Certificate{cs.ServerCert1},
			serverGetRoot: func(_ *ConnectionInfo) (*RootCertificates, error) {
				switch stage.read() {
				case 0, 1:
			...
			serverVerifyFunc: func(_ *HandshakeVerificationInfo) (*PostHandshakeVerificationResults, error) {
				return &PostHandshakeVerificationResults{}, nil
			},
		...
		// by ClientTrust2.
		{
			...
			clientGetRoot: func(_ *ConnectionInfo) (*RootCertificates, error) {
				switch stage.read() {
				case 0, 1:
	...
	for _, test := range []struct {
		...
		t.Run(test.desc, func(t *testing.T) {
			...
			serverOptions := &Options{
				...
				RequireClientCert: true,
				AdditionalPeerVerification: func(_ *HandshakeVerificationInfo) (*PostHandshakeVerificationResults, error) {
					return &PostHandshakeVerificationResults{}, nil
				},
