...
func (s) TestClientServerHandshake(t *testing.T) {
	...
	getRootCertificatesForClient := func(_ *ConnectionInfo) (*RootCertificates, error) {
	...
	verifyFuncBad := func(_ *HandshakeVerificationInfo) (*PostHandshakeVerificationResults, error) {
		return nil, fmt.Errorf("custom verification function failed")
	}
	getRootCertificatesForServer := func(_ *ConnectionInfo) (*RootCertificates, error) {
	...
	getRootCertificatesForServerBad := func(_ *ConnectionInfo) (*RootCertificates, error) {
		return nil, fmt.Errorf("bad root certificate reloading")
	}

	getRootCertificatesForClientCRL := func(_ *ConnectionInfo) (*RootCertificates, error) {
		return &RootCertificates{TrustCerts: cs.ClientTrust3}, nil
	}

	getRootCertificatesForServerCRL := func(_ *ConnectionInfo) (*RootCertificates, error) {
	...
	for _, test := range []struct {
		...
		// Expected Behavior: success
		{
			...
			clientGetCert: func(_ *tls.CertificateRequestInfo) (*tls.Certificate, error) {
			...
			clientVerificationType: CertVerification,
			serverMutualTLS:        true,
			serverGetCert: func(_ *tls.ClientHelloInfo) ([]*tls.Certificate, error) {
