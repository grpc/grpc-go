...
git grep -e 'context.Background()' --or -e 'context.TODO()' -- "*_test.go" | grep -v "benchmark/primitives/context_test.go" | grep -v "credentials/google" | grep -v "internal/transport/" | not grep -v 'context.WithTimeout(context.Background()' | not grep -v 'context.WithTimeOut(context.TODO()'
