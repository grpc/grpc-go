...
func (s) TestCSDS(t *testing.T) {
	...
	for i := range ldsTargets {
		...
		listenerAnys[i] = testutils.TestMarshalAny(t, listeners[i])
	}
	...
	for i := range rdsTargets {
		...
		routeAnys[i] = testutils.TestMarshalAny(t, routes[i])
	}
	...
	for i := range cdsTargets {
		...
		clusterAnys[i] = testutils.TestMarshalAny(t, clusters[i])
	}
	...
	for i := range edsTargets {
		...
		endpointAnys[i] = testutils.TestMarshalAny(t, endpoints[i])
	}
