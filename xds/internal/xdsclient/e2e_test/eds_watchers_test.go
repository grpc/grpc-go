...
// does not receive an EDS response for the request that it sends. We want the
...
// TestEDSWatch_ExpiryTimerFiresAfterResponse tests the case where the client
// receives an EDS response for the request that it sends. The test verifies
// that the timer is canceled and therefore no callback is invoked when the
// expiry timer's duration elapses.
func (s) TestEDSWatch_ExpiryTimerFiresAfterResponse(t *testing.T) {
	overrideFedEnvVar(t)
