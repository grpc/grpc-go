...
func newRefCounted(name string, pool *Pool, config *bootstrap.Config, watchExpiryTimeout, idleChannelExpiryTimeout time.Duration, streamBackoff func(int) time.Duration) (XDSClient, func(), error) {
	pool.mu.Lock()
	defer pool.mu.Unlock()
