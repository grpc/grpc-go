...
func (a *authority) handleADSStreamFailure(serverConfig *bootstrap.ServerConfig, err error) {
	...
	if !a.uncachedWatcherExistsLocked() {
		if a.logger.V(2) {
...
// Determines the server to fallback to, creates a channel to that server, and
// re-subscribes to all existing resources.
//
// Only executed in the context of a serializer callback.
func (a *authority) triggerFallbackOnStreamFailure(failingServerConfig *bootstrap.ServerConfig) {
	...
	a.activeChannel = fallbackChannel
...
func (a *authority) handleADSResourceUpdate(serverConfig *bootstrap.ServerConfig, rType xdsresource.Type, updates map[string]ads.DataAndErrTuple, md xdsresource.UpdateMetadata, onDone func()) {
	...
	for name, uErr := range updates {
		...
		// Log a valid update for a resource whose deletion was previously
		// ignored.
...
func (a *authority) handleRevertingToPrimaryOnUpdate(serverConfig *bootstrap.ServerConfig) {
	if a.activeChannel != nil && a.activeChannel.sc.Equal(serverConfig) {
	...
	a.activeChannel = a.xdsChannelConfigs[serverIdx]
...
func (a *authority) uncachedWatcherExistsLocked() bool {
