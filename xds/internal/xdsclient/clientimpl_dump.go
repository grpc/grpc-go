...
func appendMaps(dst, src map[xdsresource.Type]map[string]xdsresource.UpdateWithMD) {
...
func (c *clientImpl) DumpResources() map[xdsresource.Type]map[string]xdsresource.UpdateWithMD {
	c.authorityMu.Lock()
	defer c.authorityMu.Unlock()
	dumps := make(map[xdsresource.Type]map[string]xdsresource.UpdateWithMD)
