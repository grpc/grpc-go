...
func (v2c *v2Client) handleRDSResponse(resp *xdspb.DiscoveryResponse) error {
	...
	for _, r := range resp.GetResources() {
		...
		infof(v2c.parent, "Resource with name: %v, type: %T, contains: %v", rc.GetName(), rc, rc)
		...
		debugf(v2c.parent, "Resource with name %v, type %T, value %+v added to cache", rc.GetName(), cluster, cluster)
