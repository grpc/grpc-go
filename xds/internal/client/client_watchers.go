...
type watchInfo struct {
	...
	ldsCallback func(common.ListenerUpdate, error)
	rdsCallback func(common.RouteConfigUpdate, error)
	cdsCallback func(common.ClusterUpdate, error)
	edsCallback func(common.EndpointsUpdate, error)
...
func (wi *watchInfo) sendErrorLocked(err error) {
	...
	switch wi.typeURL {
	case common.V2ListenerURL:
		u = common.ListenerUpdate{}
	case common.V2RouteConfigURL:
		u = common.RouteConfigUpdate{}
	case common.V2ClusterURL:
		u = common.ClusterUpdate{}
	case common.V2EndpointsURL:
		u = common.EndpointsUpdate{}
	}
...
func (c *Client) watchLDS(serviceName string, cb func(common.ListenerUpdate, error)) (cancel func()) {
	wi := &watchInfo{
		c:           c,
...
// calling cancel()), there's a small window where the callback can be called
// after the watcher is canceled. The caller needs to handle this case.
func (c *Client) watchRDS(routeName string, cb func(common.RouteConfigUpdate, error)) (cancel func()) {
	wi := &watchInfo{
		c:           c,
...
// calling cancel()), there's a small window where the callback can be called
// after the watcher is canceled. The caller needs to handle this case.
func (c *Client) WatchService(serviceName string, cb func(common.ServiceUpdate, error)) (cancel func()) {
	c.mu.Lock()
	if len(c.ldsWatchers) != 0 {
		go cb(common.ServiceUpdate{}, fmt.Errorf("unexpected WatchService when there's another service being watched"))
		c.mu.Unlock()
		return func() {}
	}
	c.mu.Unlock()
...
type serviceUpdateWatcher struct {
	c         *Client
	ldsCancel func()
	serviceCb func(common.ServiceUpdate, error)
...
func (w *serviceUpdateWatcher) handleLDSResp(update common.ListenerUpdate, err error) {
	...
	if err != nil {
		...
		// existing RDS watch.
		w.serviceCb(common.ServiceUpdate{}, err)
		return
	}
...
func (w *serviceUpdateWatcher) handleRDSResp(update common.RouteConfigUpdate, err error) {
	...
	w.serviceCb(common.ServiceUpdate(update), nil)
}
...
// calling cancel()), there's a small window where the callback can be called
// after the watcher is canceled. The caller needs to handle this case.
func (c *Client) WatchCluster(clusterName string, cb func(common.ClusterUpdate, error)) (cancel func()) {
	wi := &watchInfo{
		c:           c,
		typeURL:     common.V2ClusterURL,
		target:      clusterName,
		cdsCallback: cb,
	}
...
// calling cancel()), there's a small window where the callback can be called
// after the watcher is canceled. The caller needs to handle this case.
func (c *Client) WatchEndpoints(clusterName string, cb func(common.EndpointsUpdate, error)) (cancel func()) {
	wi := &watchInfo{
		c:           c,
