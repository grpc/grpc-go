...
func (s) TestLDSWatch(t *testing.T) {
	...
	cancelWatch := c.watchLDS(testLDSName, func(update common.ListenerUpdate, err error) {
	...
	wantUpdate := common.ListenerUpdate{RouteConfigName: testRDSName}
	...
	if u, err := ldsUpdateCh.Receive(); err != nil || u != (ldsUpdateErr{wantUpdate, nil}) {
		t.Errorf("unexpected common.ListenerUpdate: %v, error receiving from channel: %v", u, err)
	}
	...
	if u, err := ldsUpdateCh.Receive(); err != nil || u != (ldsUpdateErr{wantUpdate, nil}) {
		t.Errorf("unexpected common.ListenerUpdate: %v, %v, want channel recv timeout", u, err)
	}
...
func (s) TestLDSTwoWatchSameResourceName(t *testing.T) {
	...
	for i := 0; i < count; i++ {
		...
		cancelLastWatch = c.watchLDS(testLDSName, func(update common.ListenerUpdate, err error) {
			ldsUpdateCh.Send(ldsUpdateErr{u: update, err: err})
		})
		if _, err := v2Client.addWatches[common.V2ListenerURL].Receive(); i == 0 && err != nil {
			t.Fatalf("want new watch to start, got error %v", err)
		}
	}
	...
	for i := 0; i < count; i++ {
		if u, err := ldsUpdateChs[i].Receive(); err != nil || u != (ldsUpdateErr{wantUpdate, nil}) {
			t.Errorf("i=%v, unexpected common.ListenerUpdate: %v, error receiving from channel: %v", i, u, err)
		}
	}
...
func (s) TestLDSThreeWatchDifferentResourceName(t *testing.T) {
	...
	for i := 0; i < count; i++ {
		ldsUpdateCh := testutils.NewChannel()
		ldsUpdateChs = append(ldsUpdateChs, ldsUpdateCh)
		c.watchLDS(testLDSName+"1", func(update common.ListenerUpdate, err error) {
			ldsUpdateCh.Send(ldsUpdateErr{u: update, err: err})
		})
		if _, err := v2Client.addWatches[common.V2ListenerURL].Receive(); i == 0 && err != nil {
			t.Fatalf("want new watch to start, got error %v", err)
	...
	c.watchLDS(testLDSName+"2", func(update common.ListenerUpdate, err error) {
	...
	wantUpdate1 := common.ListenerUpdate{RouteConfigName: testRDSName + "1"}
	wantUpdate2 := common.ListenerUpdate{RouteConfigName: testRDSName + "2"}
	v2Client.r.NewListeners(map[string]common.ListenerUpdate{
...
func (s) TestLDSWatchAfterCache(t *testing.T) {
	...
	ldsUpdateCh := testutils.NewChannel()
	c.watchLDS(testLDSName, func(update common.ListenerUpdate, err error) {
		ldsUpdateCh.Send(ldsUpdateErr{u: update, err: err})
	})
	if _, err := v2Client.addWatches[common.V2ListenerURL].Receive(); err != nil {
		t.Fatalf("want new watch to start, got error %v", err)
	...
	if n, err := v2Client.addWatches[common.V2ListenerURL].Receive(); err == nil {
...
func (s) TestLDSResourceRemoved(t *testing.T) {
	...
	wantUpdate1 := common.ListenerUpdate{RouteConfigName: testEDSName + "1"}
	wantUpdate2 := common.ListenerUpdate{RouteConfigName: testEDSName + "2"}
	v2Client.r.NewListeners(map[string]common.ListenerUpdate{
		testLDSName + "1": wantUpdate1,
		testLDSName + "2": wantUpdate2,
	})
	...
	if u, err := ldsUpdateCh1.Receive(); err != nil || ErrType(u.(ldsUpdateErr).err) != ErrorTypeResourceNotFound {
		t.Errorf("unexpected common.ListenerUpdate: %v, error receiving from channel: %v, want update with error resource not found", u, err)
	}
	...
	if u, err := ldsUpdateCh1.Receive(); err != testutils.ErrRecvTimeout {
		t.Errorf("unexpected common.ListenerUpdate: %v, want receiving from channel timeout", u)
	}
