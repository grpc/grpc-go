...
func startXDS(t *testing.T, xdsname string, v2c *client, reqChan *testutils.Channel, req *xdspb.DiscoveryRequest, preVersion string, preNonce string) {
	...
	switch xdsname {
	case "LDS":
		typeURLToWatch = common.V2ListenerURL
		nameToWatch = goodLDSTarget1
	case "RDS":
		typeURLToWatch = common.V2RouteConfigURL
		nameToWatch = goodRouteName1
	case "CDS":
		typeURLToWatch = common.V2ClusterURL
		nameToWatch = goodClusterName1
	case "EDS":
		typeURLToWatch = common.V2EndpointsURL
...
func (s) TestV2ClientAckNewWatchAfterCancel(t *testing.T) {
	...
	// Start a CDS watch.
	v2c.AddWatch(common.V2ClusterURL, goodClusterName1)
	...
	v2c.RemoveWatch(common.V2ClusterURL, goodClusterName1)
	// Wait for a request with no resource names, because the only watch was
	// removed.
	emptyReq := &xdspb.DiscoveryRequest{Node: goodNodeProto, TypeUrl: common.V2ClusterURL}
