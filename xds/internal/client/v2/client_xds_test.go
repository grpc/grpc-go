...
func (s) TestCDSHandleResponse(t *testing.T) {
	tests := []struct {
		...
		wantErr       bool
		wantUpdate    *common.ClusterUpdate
		wantUpdateErr bool
	}{
		...
		// Response contains one cluster and it is good.
		{
			...
			wantErr:       false,
			wantUpdate:    &common.ClusterUpdate{ServiceName: serviceName1, EnableLRS: true},
			wantUpdateErr: false,
		},
	}
...
func (s) TestEDSHandleResponse(t *testing.T) {
	tests := []struct {
		...
		wantErr       bool
		wantUpdate    *common.EndpointsUpdate
		wantUpdateErr bool
	}{
...
func (s) TestLDSHandleResponse(t *testing.T) {
	tests := []struct {
		...
		wantErr       bool
		wantUpdate    *common.ListenerUpdate
		wantUpdateErr bool
	}{
		...
		// Response contains one listener and it is good.
		{
			...
			wantErr:       false,
			wantUpdate:    &common.ListenerUpdate{RouteConfigName: goodRouteName1},
			wantUpdateErr: false,
		},
...
// watch.
func doLDS(t *testing.T, v2c version.APIClient, fakeServer *fakeserver.Server) {
...
func (s) TestRDSHandleResponse(t *testing.T) {
	tests := []struct {
		...
		wantErr       bool
		wantUpdate    *common.RouteConfigUpdate
		wantUpdateErr bool
	}{
		...
		// Response contains one good interesting RouteConfiguration.
		{
			...
			wantErr:       false,
			wantUpdate:    &common.RouteConfigUpdate{WeightedCluster: map[string]uint32{goodClusterName1: 1}},
			wantUpdateErr: false,
		},
	}
	for _, test := range tests {
