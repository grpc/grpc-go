...
func (s) TestRDSWatch(t *testing.T) {
	...
	cancelWatch := c.watchRDS(testRDSName, func(update common.RouteConfigUpdate, err error) {
	...
	wantUpdate := common.RouteConfigUpdate{WeightedCluster: map[string]uint32{testCDSName: 1}}
	...
	if u, err := rdsUpdateCh.Receive(); err != nil || !cmp.Equal(u, rdsUpdateErr{wantUpdate, nil}, cmp.AllowUnexported(rdsUpdateErr{})) {
		t.Errorf("unexpected common.RouteConfigUpdate: %v, error receiving from channel: %v", u, err)
	}
	...
	if u, err := rdsUpdateCh.TimedReceive(chanRecvTimeout); err != testutils.ErrRecvTimeout {
		t.Errorf("unexpected common.RouteConfigUpdate: %v, %v, want channel recv timeout", u, err)
	}
...
func (s) TestRDSTwoWatchSameResourceName(t *testing.T) {
	...
	for i := 0; i < count; i++ {
		...
		cancelLastWatch = c.watchRDS(testRDSName, func(update common.RouteConfigUpdate, err error) {
			rdsUpdateCh.Send(rdsUpdateErr{u: update, err: err})
		})
		if _, err := v2Client.addWatches[common.V2RouteConfigURL].Receive(); i == 0 && err != nil {
			t.Fatalf("want new watch to start, got error %v", err)
		}
	}
	...
	for i := 0; i < count; i++ {
		if u, err := rdsUpdateChs[i].Receive(); err != nil || !cmp.Equal(u, rdsUpdateErr{wantUpdate, nil}, cmp.AllowUnexported(rdsUpdateErr{})) {
			t.Errorf("i=%v, unexpected common.RouteConfigUpdate: %v, error receiving from channel: %v", i, u, err)
		}
	}
...
func (s) TestRDSThreeWatchDifferentResourceName(t *testing.T) {
	...
	for i := 0; i < count; i++ {
		rdsUpdateCh := testutils.NewChannel()
		rdsUpdateChs = append(rdsUpdateChs, rdsUpdateCh)
		c.watchRDS(testRDSName+"1", func(update common.RouteConfigUpdate, err error) {
			rdsUpdateCh.Send(rdsUpdateErr{u: update, err: err})
		})
		if _, err := v2Client.addWatches[common.V2RouteConfigURL].Receive(); i == 0 && err != nil {
			t.Fatalf("want new watch to start, got error %v", err)
	...
	c.watchRDS(testRDSName+"2", func(update common.RouteConfigUpdate, err error) {
	...
	wantUpdate1 := common.RouteConfigUpdate{WeightedCluster: map[string]uint32{testCDSName + "1": 1}}
	wantUpdate2 := common.RouteConfigUpdate{WeightedCluster: map[string]uint32{testCDSName + "2": 1}}
	v2Client.r.NewRouteConfigs(map[string]common.RouteConfigUpdate{
...
func (s) TestRDSWatchAfterCache(t *testing.T) {
	...
	rdsUpdateCh := testutils.NewChannel()
	c.watchRDS(testRDSName, func(update common.RouteConfigUpdate, err error) {
		rdsUpdateCh.Send(rdsUpdateErr{u: update, err: err})
	})
	if _, err := v2Client.addWatches[common.V2RouteConfigURL].Receive(); err != nil {
		t.Fatalf("want new watch to start, got error %v", err)
	...
	if n, err := v2Client.addWatches[common.V2RouteConfigURL].Receive(); err == nil {
