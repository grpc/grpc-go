...
func (s) TestRDSWatch(t *testing.T) {
	...
	cancelWatch := client.WatchRoute(testRDSName, func(update RouteConfigUpdate, err error) {
...
func (s) TestRDSTwoWatchSameResourceName(t *testing.T) {
	...
	for i := 0; i < count; i++ {
		...
		cancelLastWatch = client.WatchRoute(testRDSName, func(update RouteConfigUpdate, err error) {
			rdsUpdateCh.Send(rdsUpdateErr{u: update, err: err})
		})
...
func (s) TestRDSThreeWatchDifferentResourceName(t *testing.T) {
	...
	for i := 0; i < count; i++ {
		rdsUpdateCh := testutils.NewChannel()
		rdsUpdateChs = append(rdsUpdateChs, rdsUpdateCh)
		client.WatchRoute(testRDSName+"1", func(update RouteConfigUpdate, err error) {
			rdsUpdateCh.Send(rdsUpdateErr{u: update, err: err})
		})
	...
	client.WatchRoute(testRDSName+"2", func(update RouteConfigUpdate, err error) {
...
func (s) TestRDSWatchAfterCache(t *testing.T) {
	...
	rdsUpdateCh := testutils.NewChannel()
	client.WatchRoute(testRDSName, func(update RouteConfigUpdate, err error) {
		rdsUpdateCh.Send(rdsUpdateErr{u: update, err: err})
	})
	if _, err := apiClient.addWatches[RouteConfigResource].Receive(ctx); err != nil {
		t.Fatalf("want new watch to start, got error %v", err)
