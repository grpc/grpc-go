...
// xdsClient contains only the xds_client methods needed by LRS balancer. It's
// defined so we can override xdsclient in tests.
...
func (b *clusterImplBalancer) UpdateClientConnState(s balancer.ClientConnState) error {
	...
	if b.config == nil || b.config.ChildPolicy.Name != newConfig.ChildPolicy.Name {
		...
		b.childLB = bb.Build(b, b.buildOpts)
	}
	b.config = newConfig
