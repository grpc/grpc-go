...
type clusterHandlerUpdate struct {
	...
	securityCfg *resource.SecurityConfig
	...
	updates []resource.ClusterUpdate
	err     error
}
...
func (c *clusterNode) constructClusterUpdate() ([]resource.ClusterUpdate, error) {
	...
	if c.clusterUpdate.ClusterType != resource.ClusterTypeAggregate {
		return []resource.ClusterUpdate{c.clusterUpdate}, nil
	}

	// If an aggregate construct a list by recursively calling down to all of
	// it's children.
	var childrenUpdates []resource.ClusterUpdate
	for _, child := range c.children {
...
func (c *clusterNode) handleResp(clusterUpdate resource.ClusterUpdate, err error) {
