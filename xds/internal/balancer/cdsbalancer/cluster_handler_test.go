...
func (s) TestSuccessCaseLeafNode(t *testing.T) {
	tests := []struct {
		name          string
		clusterName   string
		clusterUpdate resource.ClusterUpdate
		lbPolicy      *resource.ClusterLBPolicyRingHash
	}{
		{
			name:        "test-update-root-cluster-EDS-success",
			clusterName: edsService,
			clusterUpdate: resource.ClusterUpdate{
				ClusterType: resource.ClusterTypeEDS,
				ClusterName: edsService,
			},
		},
		{
			...
			clusterUpdate: resource.ClusterUpdate{
				ClusterType: resource.ClusterTypeLogicalDNS,
				ClusterName: logicalDNSService,
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			case chu := <-ch.updateChannel:
				if diff := cmp.Diff(chu.updates, []resource.ClusterUpdate{test.clusterUpdate}); diff != "" {
...
func (s) TestSuccessCaseLeafNodeThenNewUpdate(t *testing.T) {
	tests := []struct {
		...
		clusterName      string
		clusterUpdate    resource.ClusterUpdate
		newClusterUpdate resource.ClusterUpdate
	}{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			select {
			case chu := <-ch.updateChannel:
				if diff := cmp.Diff(chu.updates, []resource.ClusterUpdate{test.newClusterUpdate}); diff != "" {
					t.Fatalf("got unexpected cluster update, diff (-got, +want): %v", diff)
				}
			case <-ctx.Done():
				t.Fatal("Timed out waiting for update from updateChannel.")
...
func (s) TestUpdateRootClusterAggregateSuccess(t *testing.T) {
	...
	fakeClient.InvokeWatchClusterCallback(resource.ClusterUpdate{
		ClusterType:             resource.ClusterTypeAggregate,
	...
	select {
	case chu := <-ch.updateChannel:
		if diff := cmp.Diff(chu.updates, []resource.ClusterUpdate{{
			ClusterType: resource.ClusterTypeEDS,
			ClusterName: edsService,
		}, {
			ClusterType: resource.ClusterTypeLogicalDNS,
...
func (s) TestHandleRespInvokedWithError(t *testing.T) {
	...
	fakeClient.InvokeWatchClusterCallback(resource.ClusterUpdate{}, errors.New("some error"))
	select {
	case chu := <-ch.updateChannel:
