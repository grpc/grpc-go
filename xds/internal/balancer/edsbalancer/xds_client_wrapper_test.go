...
func (s) TestClientWrapperWatchEDS(t *testing.T) {
	...
	cw := newXDSClientWrapper(nil, nil, nil, balancer.BuildOptions{Target: resolver.Target{Endpoint: testServiceName}}, nil)
	defer cw.close()
...
func (s) TestClientWrapperHandleUpdateError(t *testing.T) {
	...
	cw := newXDSClientWrapper(nil, newEDS, nil, balancer.BuildOptions{Target: resolver.Target{Endpoint: testServiceName}}, nil)
	defer cw.close()
