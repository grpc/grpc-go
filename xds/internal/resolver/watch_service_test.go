...
func (s) TestFindBestMatchingVirtualHost(t *testing.T) {
	var (
		oneExactMatch = &resource.VirtualHost{
			Domains: []string{"foo.bar.com"},
		}
		oneSuffixMatch = &resource.VirtualHost{
			Domains: []string{"*.bar.com"},
		}
		onePrefixMatch = &resource.VirtualHost{
			Domains: []string{"foo.bar.*"},
		}
		oneUniversalMatch = &resource.VirtualHost{
			Domains: []string{"*"},
		}
		longExactMatch = &resource.VirtualHost{
			Domains: []string{"v2.foo.bar.com"},
		}
		multipleMatch = &resource.VirtualHost{
			Domains: []string{"pi.foo.bar.com", "314.*", "*.159"},
		}
		vhs = []*resource.VirtualHost{oneExactMatch, oneSuffixMatch, onePrefixMatch, oneUniversalMatch, longExactMatch, multipleMatch}
	)

	tests := []struct {
		name   string
		host   string
		vHosts []*resource.VirtualHost
		want   *resource.VirtualHost
	}{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := resource.FindBestMatchingVirtualHost(tt.host, tt.vHosts); !cmp.Equal(got, tt.want, cmp.Comparer(proto.Equal)) {
...
func (s) TestServiceWatch(t *testing.T) {
	...
	xdsC.InvokeWatchListenerCallback(resource.ListenerUpdate{RouteConfigName: routeStr}, nil)
	waitForWatchRouteConfig(ctx, t, xdsC, routeStr)

	wantUpdate := serviceUpdate{virtualHost: &resource.VirtualHost{Domains: []string{"target"}, Routes: []*resource.Route{{Prefix: newStringP(""), WeightedClusters: map[string]resource.WeightedCluster{cluster: {Weight: 1}}}}}}
	...
	wantUpdate2 := serviceUpdate{virtualHost: &resource.VirtualHost{Domains: []string{"target"},
		Routes: []*resource.Route{{
			Path:             newStringP(""),
			WeightedClusters: map[string]resource.WeightedCluster{cluster: {Weight: 1}},
		}},
	}}
	xdsC.InvokeWatchRouteConfigCallback("", resource.RouteConfigUpdate{
		VirtualHosts: []*resource.VirtualHost{
			{
				Domains: []string{targetStr},
				Routes:  []*resource.Route{{Path: newStringP(""), WeightedClusters: map[string]resource.WeightedCluster{cluster: {Weight: 1}}}},
			},
			{
...
func (s) TestServiceWatchLDSUpdate(t *testing.T) {
	...
	xdsC.InvokeWatchListenerCallback(resource.ListenerUpdate{RouteConfigName: routeStr + "2"}, nil)
	...
	wantUpdate2 := serviceUpdate{virtualHost: &resource.VirtualHost{Domains: []string{"target"}, Routes: []*resource.Route{{Prefix: newStringP(""), WeightedClusters: map[string]resource.WeightedCluster{cluster + "2": {Weight: 1}}}}}}
	xdsC.InvokeWatchRouteConfigCallback(routeStr+"2", resource.RouteConfigUpdate{
		VirtualHosts: []*resource.VirtualHost{
			{
				Domains: []string{targetStr},
				Routes:  []*resource.Route{{Prefix: newStringP(""), WeightedClusters: map[string]resource.WeightedCluster{cluster + "2": {Weight: 1}}}},
			},
		},
	}, nil)
	if err := verifyServiceUpdate(ctx, serviceUpdateCh, wantUpdate2); err != nil {
...
func (s) TestServiceWatchLDSUpdateMaxStreamDuration(t *testing.T) {
	...
	defer cancel()
	waitForWatchListener(ctx, t, xdsC, targetStr)
	xdsC.InvokeWatchListenerCallback(resource.ListenerUpdate{RouteConfigName: routeStr, MaxStreamDuration: time.Second}, nil)
	waitForWatchRouteConfig(ctx, t, xdsC, routeStr)

	wantUpdate := serviceUpdate{virtualHost: &resource.VirtualHost{Domains: []string{"target"}, Routes: []*resource.Route{{
		Prefix:           newStringP(""),
		WeightedClusters: map[string]resource.WeightedCluster{cluster: {Weight: 1}}}}},
	...
	wantUpdate2 := serviceUpdate{virtualHost: &resource.VirtualHost{Domains: []string{"target"}, Routes: []*resource.Route{{Prefix: newStringP(""), WeightedClusters: map[string]resource.WeightedCluster{cluster: {Weight: 1}}}}}}
	xdsC.InvokeWatchListenerCallback(resource.ListenerUpdate{RouteConfigName: routeStr}, nil)
	if err := verifyServiceUpdate(ctx, serviceUpdateCh, wantUpdate2); err != nil {
		t.Fatal(err)
	}
	...
	// RDS update.
	wantUpdate3 := serviceUpdate{virtualHost: &resource.VirtualHost{Domains: []string{"target"}, Routes: []*resource.Route{{
		Prefix:           newStringP(""),
		WeightedClusters: map[string]resource.WeightedCluster{cluster + "2": {Weight: 1}}}},
	}}
	xdsC.InvokeWatchRouteConfigCallback("", resource.RouteConfigUpdate{
		VirtualHosts: []*resource.VirtualHost{
			{
...
func (s) TestServiceWatchInlineRDS(t *testing.T) {
	...
	wantVirtualHosts2 := &resource.VirtualHost{Domains: []string{"target"},
		Routes: []*resource.Route{{
			Path:             newStringP(""),
			WeightedClusters: map[string]resource.WeightedCluster{cluster: {Weight: 1}},
		}},
	...
	xdsC.InvokeWatchListenerCallback(resource.ListenerUpdate{InlineRouteConfig: &resource.RouteConfigUpdate{
		VirtualHosts: []*resource.VirtualHost{wantVirtualHosts2},
	}}, nil)
