...
var (
	...
	enableOtelTracing    = flag.Bool("enable_otel_tracing", false, "Whether to enable OpenTelemetry tracing")
	enableOpenTelemetry  = flag.Bool("enable_opentelemetry", false, "Whether to enable OpenTelemetry")
	enableTcpMetrics     = flag.Bool("enable_tcp_metrics", false, "Whether to enable TCP metrics")
	otlpCollectorAddress = flag.String("otlp_collector_address", "", "The OTLP collector address")
...
func main() {
	...
	if *enableOtelTracing || *enableOpenTelemetry || *otlpCollectorAddress != "" || os.Getenv("OTEL_EXPORTER_OTLP_ENDPOINT") != "" {
		ctx := context.Background()
		var exporterOpts []otlptracegrpc.Option
		addr := *otlpCollectorAddress
		if addr == "" {
