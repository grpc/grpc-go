...
func (cs *clientStream) SendMsg(m interface{}) (err error) {
	if cs.tracing {
		cs.mu.Lock()
		if cs.trInfo.tr != nil {
			cs.trInfo.tr.LazyLog(newPayload(true, m), true)
		}
		cs.mu.Unlock()
	}
...
func (cs *clientStream) RecvMsg(m interface{}) (err error) {
	...
	if err == nil {
		if cs.tracing {
			cs.mu.Lock()
			if cs.trInfo.tr != nil {
				cs.trInfo.tr.LazyLog(newPayload(false, m), true)
			}
			cs.mu.Unlock()
		}
		if inPayload != nil {
...
func (ss *serverStream) SendMsg(m interface{}) (err error) {
	defer func() {
		if ss.trInfo != nil {
			...
			if ss.trInfo.tr != nil {
				if err == nil {
					ss.trInfo.tr.LazyLog(newPayload(true, m), true)
				} else {
...
func (ss *serverStream) RecvMsg(m interface{}) (err error) {
	defer func() {
		if ss.trInfo != nil {
			...
			if ss.trInfo.tr != nil {
				if err == nil {
					ss.trInfo.tr.LazyLog(newPayload(false, m), true)
				} else if err != io.EOF {
					ss.trInfo.tr.LazyLog(&fmtStringer{"%v", []interface{}{err}}, true)
					ss.trInfo.tr.SetError()
				}
