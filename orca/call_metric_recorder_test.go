...
// Test_E2E_CustomBackendMetrics_PerRPC_Unary tests the injection of custom
...
func (s) Test_E2E_CustomBackendMetrics_PerRPC_Unary(t *testing.T) {
	...
	// A stub server whose unary handler injects custom metrics. It overwrites
	...
	// Extract and compare the load report from the trailer metadata.
	...
	if !cmp.Equal(gotProto, wantProto, cmp.Comparer(proto.Equal)) {
		t.Fatalf("Received load report in trailer: %s, want: %s", pretty.ToJSON(gotProto), pretty.ToJSON(wantProto))
	}
}

// Test_E2E_CustomBackendMetrics_PerRPC_Streaming tests the injection of custom
...
func (s) Test_E2E_CustomBackendMetrics_PerRPC_Streaming(t *testing.T) {
	...
	// A stub server whose streaming handler injects custom metrics. It overwrites
	// one of the values injected above, by the interceptor.
	srv := stubserver.StubServer{
