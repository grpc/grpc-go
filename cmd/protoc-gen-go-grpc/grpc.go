...
func generateFile(gen *protogen.Plugin, file *protogen.File) *protogen.GeneratedFile {
	...
	genCopyrightComments(g, *file.Proto.Name)
...
func genCopyrightComments(g *protogen.GeneratedFile, protoPath string) {
	str, _ := os.ReadFile(protoPath)

	var rgx *regexp.Regexp
	start := string(str[1])

	if start == "/" {
		rgx = regexp.MustCompile(`\/\/ Copyright([\s\S]*?under the License\.)`)
	} else if start == "*" {
		rgx = regexp.MustCompile(`\/\*([\s\S]*?)\*\/`)
	}

	matches := rgx.FindString(string(str))
