...
func makeFallbackClientCreds(t *testing.T, isMTLS bool) credentials.TransportCredentials {
	...
	if isMTLS {
		...
		creds = credentials.NewTLS(&tls.Config{
			...
			RootCAs:    cp,
			// In MTLS the client needs to configure a certificate
...
func (s) TestClientCredsSuccess(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.desc, func(t *testing.T) {
			...
			opts := ClientOptions{FallbackCreds: makeFallbackClientCreds(t, test.isMTLS)}
			creds, err := NewClientCredentials(opts)
			if err != nil {
				t.Fatalf("NewClientCredentials(%v) failed: %v", opts, err)
			}
