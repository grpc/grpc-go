...
// storeKey acts the key to the map of providers maintained by the store.
...
// no registered builder is found, a non-nil error is returned.
func (ps *Store) GetProvider(key Key) (Provider, error) {
	ps.mu.Lock()
	defer ps.mu.Unlock()
	...
	provider, err := b.Build(BuildOptions{Config: key.Config})
	if err != nil {
		return nil, err
	}
	wp := &wrappedProvider{
