...
// TestPickFirstLeaf_OneBackend tests the most basic scenario for pick_first. It
// brings up a single backend and verifies that all RPCs get routed to it.
func (s) TestPickFirstLeaf_OneBackend(t *testing.T) {
...
// TestPickFirstLeaf_MultipleBackends tests the scenario with multiple backends and
// verifies that all RPCs get routed to the first one.
func (s) TestPickFirstLeaf_MultipleBackends(t *testing.T) {
	cc, r, backends := setupPickFirstLeaf(t, 2)
...
// TestPickFirstLeaf_OneServerDown tests the scenario where we have multiple
...
func (s) TestPickFirstLeaf_OneServerDown(t *testing.T) {
	cc, r, backends := setupPickFirstLeaf(t, 2)
...
// TestPickFirstLeaf_AllServersDown tests the scenario where we have multiple
...
func (s) TestPickFirstLeaf_AllServersDown(t *testing.T) {
	cc, r, backends := setupPickFirstLeaf(t, 2)
...
// TestPickFirstLeaf_AddressesRemoved tests the scenario where we have multiple
...
func (s) TestPickFirstLeaf_AddressesRemoved(t *testing.T) {
	cc, r, backends := setupPickFirstLeaf(t, 3)
...
// TestPickFirstLeaf_NewAddressWhileBlocking tests the case where pick_first is
...
func (s) TestPickFirstLeaf_NewAddressWhileBlocking(t *testing.T) {
	cc, r, backends := setupPickFirstLeaf(t, 2)
	addrs := stubBackendsToResolverAddrs(backends)
	r.UpdateState(resolver.State{Addresses: addrs})
...
// TestPickFirstLeaf_StickyTransientFailure tests the case where pick_first is
...
func (s) TestPickFirstLeaf_StickyTransientFailure(t *testing.T) {
...
func (s) TestPickFirstLeaf_ShuffleAddressList(t *testing.T) {
	const serviceConfig = `{"loadBalancingConfig": [{"pick_first_leaf":{ "shuffleAddressList": true }}]}`
...
func (s) TestPickFirstLeaf_ParseConfig_Success(t *testing.T) {
	// Install a shuffler that always reverses two entries.
	origShuf := internal.ShuffleAddressListForTesting
	defer func() { internal.ShuffleAddressListForTesting = origShuf }()
	internal.ShuffleAddressListForTesting = func(n int, f func(int, int)) {
	...
	tests := []struct {
	...
	}{
		{
			...
			serviceConfig: `{"loadBalancingConfig": [{"pick_first_leaf":{}}]}`,
			wantFirstAddr: true,
		},
		{
			name:          "empty good pickfirst config",
			serviceConfig: `{"loadBalancingConfig": [{"pick_first_leaf":{ "shuffleAddressList": true }}]}`,
			wantFirstAddr: false,
		},
	}
...
func (s) TestPickFirstLeaf_ParseConfig_Failure(t *testing.T) {
	...
	const sc = `{"loadBalancingConfig": [{"pick_first_leaf":{ "shuffleAddressList": 666 }}]}`
...
// setupPickFirstLeafWithListenerWrapper is very similar to setupPickFirstLeaf, but uses
// a wrapped listener that the test can use to track accepted connections.
func setupPickFirstLeafWithListenerWrapper(t *testing.T, backendCount int, opts ...grpc.DialOption) (*grpc.ClientConn, *manual.Resolver, []*stubserver.StubServer, []*testutils.ListenerWrapper) {
	t.Helper()
...
// TestPickFirstLeaf_AddressUpdateWithAttributes tests the case where an address
...
func (s) TestPickFirstLeaf_AddressUpdateWithAttributes(t *testing.T) {
	cc, r, backends, listeners := setupPickFirstLeafWithListenerWrapper(t, 2)
...
// TestPickFirstLeaf_AddressUpdateWithBalancerAttributes tests the case where an
...
func (s) TestPickFirstLeaf_AddressUpdateWithBalancerAttributes(t *testing.T) {
	cc, r, backends, listeners := setupPickFirstLeafWithListenerWrapper(t, 2)
...
func (s) TestPickFirstLeaf_ResolverError_NoPreviousUpdate(t *testing.T) {
	cc, r, _ := setupPickFirstLeaf(t, 0)
...
// good update.
func (s) TestPickFirstLeaf_ResolverError_WithPreviousUpdate_Ready(t *testing.T) {
	cc, r, backends := setupPickFirstLeaf(t, 1)
...
func (s) TestPickFirstLeaf_ResolverError_WithPreviousUpdate_Connecting(t *testing.T) {
	lis, err := testutils.LocalTCPListener()
	if err != nil {
...
func (s) TestPickFirstLeaf_ResolverError_WithPreviousUpdate_TransientFailure(t *testing.T) {
	lis, err := testutils.LocalTCPListener()
	if err != nil {
		t.Fatalf("net.Listen() failed: %v", err)
	}
...
func (s) TestPickFirstLeaf_ResolverError_ZeroAddresses_WithPreviousUpdate(t *testing.T) {
	cc, r, backends := setupPickFirstLeaf(t, 1)
